...
<?php Pjax::begin(); ?>    
    <p>
        <?= Html::a('Create Address Type', ['create', $this->params['model'].'[type]' => $searchModel->type, 
            $this->params['model'].'[group_id]' => $searchModel->is_group ? $searchModel->group_id : null], ['class' => 'btn btn-success', 'data-toggle' => 'xmodal', 'data-target' => '#myModal']) ?>
        <?= ''/*Html::a('Create Group', ['creategroup', 'AddressType[group_id]' => $searchModel->is_group ? $searchModel->group_id : null], ['class' => 'btn btn-success'])*/ ?>
    </p>
    <?php 
    ...
    for ($i = 0; $i < strlen($str); $i ++) {
        ...
        //echo Html::a($str[$i], ['index', 'search' => ($searchModel->search == $str[$i] ? '' : $str[$i])], ['class' => 'btn '.($searchModel->search == $str[$i]? 'btn-primary' : 'btn-success')]);
        }
    ?>
...
<?= $a = LeveledGridView::widget([
        ...
        'columns' => [
            ...
            ['attribute' => 'type', 'filter' => ['registry'=>'registry', 'logistic'=>'logistic', 'other'=>'other'], 'value' => function ($data) {return $data->is_group ? '' : $data->type;}],
